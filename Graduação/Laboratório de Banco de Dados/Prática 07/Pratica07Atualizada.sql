...
BEGIN
	...
	CASE upper(p_identificador)
		...
		--UPDATE
		WHEN 'UPDATE' THEN
			comando := 'UPDATE ' || upper(p_nomeTabela) || ' SET ' || upper(p_nomeAtributo) || ' = ' || p_novoValorAtributo || ' WHERE ' || upper(p_nomeTabela) || '.' || upper(p_nomeAtributoChave) || ' = ' || p_valorAtributoChave;
...
CREATE OR REPLACE FUNCTION varianciaGols2Selecoes(p_selecao Selecao.nomeNacao%TYPE,
