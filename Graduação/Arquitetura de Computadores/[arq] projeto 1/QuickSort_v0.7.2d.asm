...
main:
			...
			move $t1, $s2 #carrega o endereco inicial do vetor ordenado para $t1
			
loop_scan:	li $v0, 5 #atribui 5 para $v0. Codigo para ler inteiro
			syscall
			sw $v0, 0($t0) #armazena em $v0 o valor de $t0 + 0
			sw $v0, 0($t1) #armazena em $v0 o valor de $t0 + 0
			...
			addi $t1, $t1, 4 #soma 4 em $t0 para poder acessar 4 bytes para frente na memoria na proxima iteracao
			...
			li $v0, 4
			la $a0, str_ori #imprime "Vetor original:"
			syscall
...
loop_print: lw $a0, 0($t0) #armazena em $a0 o valor de %t0 + 0
			...
			li $v0, 4
			la $a0, str_nline #pula uma linha
			syscall
			
			#Chamada da funcao quicksort
			move $a0, $s2 #move o endereco do inicio do vetor para $a0
			...
			li $v0, 4
			la $a0, str_ord #imprime "Vetor ordenado:"
			syscall
...
#
quicksort:
					bge $a1, $a2, end_quicksort #sai do quicksort se a posicao da esquerda for maior ou igual que a da direita
					...
					move $t1, $a2 #salva o valor do $a2 em $t1
					...
					addi $t0, $t0, 8 #$t0 + 8 (numero de bytes para o proximo numero do vetor) para comecar um novo loop do quicksort
					move $a2, $t1 #retorna o valor da direita para $a2
...
vect_part:
			...
			move $t7, $a1 #carrega baixo em $t7
			move $t8, $a2 #carrega cima em $t8

loop_part:	bge $t7, $t8, exit_loop_part #while(baixo < cima)

loop_part_low: move $t4, $a0 #while( v[baixo] >= a && baixo < dir )
			add $t4, $t4, $t7 #carrega o endereco de baixo em $t4
			lw $t5, 0($t4) #carrega o valor de baixo em $t5
			sge $t9, $t5, $t6 # coloca 1 em $t9 se v[baixo] >= a
			slt $t4, $t7, $a2 # coloca 1 em $t4 se baixo < dir
			and $t5, $t4, $t9 # coloca 1 em $t5 se v[baixo] >= a && baixo < dir
			beq $t5, $zero, loop_part_high # se a v[baixo] >= a && baixo < dir for falso sai do loop
			addi $t7, $t7, 4 #proxima posicao de baixo no vetor
			j loop_part_low

loop_part_high:move $t4, $a0 #while( v[cima] < a )
			add $t4, $t4, $t8 #carrega o endereco de cima em $t4
			lw $t5, 0($t4) #carrega o valor de cima em $t5
			bge $t5, $t6, swap_condition #equivalente a v[cima] < a
			addi $t8, $t8, -4 #posicao anterior de cima do vetor
			j loop_part_high

swap_condition: bge $t7, $t8, no_swap #if( baixo < cima )
			...
			move $a1, $t7 #atualiza baixo
			move $a2, $t8 #atualiza cima
...
exit_loop_part:	move $t4, $a0
			add $t4, $t4, $t8 #carrega o endereco de cima para $t4
			lw $t5, 0($t4) #carrega o valor de cima para $t5
			move $t0, $a0 #carrega o endereco inicial do vetor para $t0
			add $t0, $t0, $a1 #soma o numero de bytes para a posicao baixo ($a1) do vetor
			sw $t5, 0($t0) #v[esq] = v[cima] | o vetor particionado da esquerda recebe como valor o de cima
			move $t0, $a0 
			add $t0, $t8, $t0 #soma o numero de bytes para a posicao cima ($t8) do vetor
 			sw $t6, 0($t0) #v[cima] = a | o vetor particionado de cima recebe como valor o de a
...
#
# Parametros:
#	- $a0 = posicao do inicio do vetor
#	- $a1 = posicao na memoria A
#	- $a2 = posicao na memoria B
# Retorno:
#	Os valores na memoria serão trocados entre A e B
swap:
			move $t0, $a0 #carrega o endereco inicial do vetor em $a0
			add $t0, $a1, $t0 #soma os bytes para chegar na primeira posicao do vetor a ser trocada
			lw $t1, 0($t0) #carrega o conteudo do primeira posicao do vetor para $t1
			move $t0, $a0 #carrega o endereco inicial do vetor em $a0
			add $t0, $a2, $t0 #soma os bytes para chegar na segunda posicao do vetor a ser trocada
			lw $t2, 0($t0) #carrega o conteudo da segunda posicao do vetor para $t2
			move $t0, $a0 #carrega o endereco inicial do vetor em $a0
			add $t0, $a1, $t0 #soma os bytes para chegar na primeira posicao do vetor a ser trocada
			sw $t2, 0($t0) #salva o conteudo da segunda posicao para a primeira posicao
			move $t0, $a0 #carrega o endereco inicial do vetor em $a0
			add $t0, $a2, $t0 #soma os bytes para chegar na segunda posicao do vetor a ser trocada
			sw $t1, 0($t0) #salva o conteudo da primeira posicao para a segunda posicao
			jr $ra