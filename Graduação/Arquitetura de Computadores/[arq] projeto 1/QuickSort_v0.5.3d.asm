...
# Changes:
#   - COMENTADO
#	- Função swap pronta. Ainda não foi testada.
#	- Tentativa de fazer a função quicksort. Ainda não foi testada.
#	- Chamada da função quicksort implementada.
#	- Imprimir a função ordenada implementada.
#
# Atençao:
...
loop_scan:	li $v0, 5 #atribui 5 para $v0. Codigo para ler inteiro
			syscall
			sw $v0, 0($t0) #armazena em $v0 o valor que está na memória de $t0 + 0
...
loop_print: lw $a0, 0($t0) #armazena em $a0 o valor que está na memória de %t0 + 0
			...
			addi $a1, $a1, 0 #$a1 recebe 0, a posicao inicial do vetor
			move $t0, $s0 #move N para $t0 para nao perder o valor
			addi $t0, $t0, -1 #diminui o valor de $t0 em 1 para indicar o final do vetor
			move $a2, $t0 #move $t0 para $a2, tendo assim a posicao final do vetor no registrador $a2
...
#
quicksort:
					sgt $t0, $a1, $a2 #caso o valor da direita seja maior que o da esquerda, coloca 1 em $t0.
					addi $t4, $t4, 1 #para comparacao com $t0
					beq $t0, $zero, end_quicksort #finaliza caso $t0 seja igual a 0
					
start_quicksort:	move $t0, $v0 #move o valor de $v0 para $t0, evitando que ele se perca
					addi $t0, $t0, -1 #$t0 - 1 para comecar um novo loop do quicksort
